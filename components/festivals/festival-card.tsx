'use client';

import Image from 'next/image';
import Link from 'next/link';
import { Calendar, MapPin, Users, ArrowRight } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Festival } from '@/lib/festivals-data';

interface FestivalCardProps {
  festival: Festival;
  onExplore?: (festival: Festival) => void;
}

export function FestivalCard({ festival, onExplore }: FestivalCardProps) {
  return (
    <div
      className="group"
    >
      <Card className="overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 bg-white">
        {/* Image Container */}
        <div className="relative aspect-[4/3] overflow-hidden">
          <Image
            src={festival.image_url}
            alt={festival.name}
            fill
            className="object-cover transition-transform duration-500 group-hover:scale-110"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-60" />
          
          {/* Month Badge */}
          <div className="absolute top-4 left-4">
            <Badge className="bg-secondary text-secondary-foreground hover:bg-secondary/90 px-3 py-1 text-sm font-semibold">
              {festival.month}
            </Badge>
          </div>
          
          {/* Location Badge */}
          <div className="absolute bottom-4 left-4 flex items-center gap-1 text-white/90 text-sm">
            <MapPin className="h-4 w-4" />
            <span className="font-medium">{festival.location}</span>
          </div>
        </div>
        
        {/* Content */}
        <CardContent className="p-6">
          <h3 className="font-serif text-xl font-bold mb-2 group-hover:text-primary transition-colors">
            {festival.name}
          </h3>
          
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
            <Calendar className="h-4 w-4" />
            <span>{festival.festival_date}</span>
          </div>
          
          {festival.people && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
              <Users className="h-4 w-4" />
              <span>{festival.people}</span>
            </div>
          )}
          
          <p className="text-muted-foreground text-sm line-clamp-2 mb-4">
            {festival.description}
          </p>
          
          {/* Highlights */}
          {festival.highlights && festival.highlights.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-4">
              {festival.highlights.slice(0, 2).map((highlight, i) => (
                <span 
                  key={i} 
                  className="text-xs bg-muted px-2 py-1 rounded-full text-muted-foreground"
                >
                  {highlight}
                </span>
              ))}
              {festival.highlights.length > 2 && (
                <span className="text-xs bg-muted px-2 py-1 rounded-full text-muted-foreground">
                  +{festival.highlights.length - 2} more
                </span>
              )}
            </div>
          )}
          
          {/* Actions */}
          <div className="flex gap-2">
            <Button 
              onClick={() => onExplore?.(festival)}
              className="flex-1 group-hover:bg-primary transition-colors"
            >
              Explore Festival
              <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
            </Button>
            <Button variant="outline" size="icon" asChild>
              <a 
                href={`https://wa.me/233241234567?text=Hi! I'd like to attend the ${festival.name}.`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Get more info via WhatsApp"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
              </a>
            </Button>
          </div>
        </CardContent>
      </Card>
    </div>
  );
}
